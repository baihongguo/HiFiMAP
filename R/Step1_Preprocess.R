...
N.randomvec <- 100
ibd <- get(load(global.kinship.file))
pheno <- fread(pheno.file, data.table = FALSE)
pheno <- subset(pheno, id %in% rownames(ibd))
pheno <- pheno[!apply(is.na(pheno), 1, any), ]
cat("Effective sample size:", nrow(pheno), "\n")

obj <- try(glmmkin(as.formula(paste0("pheno ~ covar")), data = pheno, id = "id", kins = ibd, family = gaussian(link = "identity"), verbose = T))
